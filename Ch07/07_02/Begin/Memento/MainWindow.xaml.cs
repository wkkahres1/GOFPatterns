...
namespace Memento
{
    ...
    public partial class MainWindow : Window
    {
        ...
        {
            // TODO
            // get text in from text box
            string text = theStatement.Text;

            //set Memento Value
            originator.set(text); //pass in current text value

            //adding and storing content into the memento
            caretaker.addMemento(originator.storeInMemento());

            //keep track of files (number of statements saved)
            saveFiles++;
            currentStatement++;
